...
from ..services.refunds import calculate_refund_amount_cents, calculate_refund_percent, log_refund
...
# Guards the read-check-then-write sequences below (conflict/quota check +
# insert, and cancel status check + refund) so concurrent requests can't race
# each other into double-booking, exceeding quota, or double-refunding.
...
def create_booking(
    ...
    try:
        ...
        raise AppError(400, "INVALID_BOOKING_WINDOW", "Invalid start_time or end_time format")
    now = datetime.utcnow()
...
def cancel_booking(
    ...
    with _booking_lock:
        ...
        refund_percent = calculate_refund_percent(notice)
        refund_amount_cents = calculate_refund_amount_cents(booking.price_cents, refund_percent)
